...
let rec equiv_assoc_cas t =
  ...
  | Term_bin (WEDGE, Term_ent (op_l, id_l), Term_ent (op_r, id_r)) -> [t]
  | Term_bin (WEDGE, Term_ent (op_l, id_l), Term_una (op_r, t_r)) -> [t]
  | Term_bin (WEDGE, Term_una (op_l, t_l), Term_ent (op_r, id_r)) -> [t]
...
and assoc_cas_family l r =
  let rec assoc_cas_family_r l r =
    let rec assoc_cas_sft2_r l r =
      match l with
        Term_ent (op, id) -> (l, r)
      | Term_una (op, l1) -> (l, r)
      | Term_bin (WEDGE, ll, Term_ent (op_lr, id_lr)) -> (ll, Term_bin (WEDGE, Term_ent (op_lr, id_lr), r))
  ...
  let rec assoc_cas_family_l l r =
    let rec assoc_cas_sft2_l l r =
      match r with
        Term_ent (op, id) -> (l, r)
      | Term_una (op, r1) -> (l, r)
      | Term_bin (WEDGE, Term_ent (op_rl, id_rl), rr) -> (Term_bin (WEDGE, l, Term_ent (op_rl, id_rl)), rr)
  ...
  | Term_bin (WEDGE, ll, lr) -> (match r with
                                   Term_ent (op_r, id) -> (assoc_cas_family_r l r)
...
let rec equiv_assoc_par t =
  ...
  | Term_bin (VEE, Term_ent (op_l, id_l), Term_ent (op_r, id_r)) -> [t]
  | Term_bin (VEE, Term_ent (op_l, id_l), Term_una (op_r, t_r)) -> [t]
  | Term_bin (VEE, Term_una (op_l, t_l), Term_ent (op_r, id_r)) -> [t]
...
and assoc_par_family l r =
  let rec assoc_par_family_r l r =
    let rec assoc_par_sft2_r l r =
      match l with
        Term_ent (op, id) -> (l, r)
      | Term_una (op, l1) -> (l, r)
      | Term_bin (VEE, ll, Term_ent (op_lr, id_lr)) -> (ll, Term_bin (VEE, Term_ent (op_lr, id_lr), r))
  ...
  let rec assoc_par_family_l l r =
    let rec assoc_par_sft2_l l r =
      match r with
        Term_ent (op, id) -> (l, r)
      | Term_una (op, r1) -> (l, r)
      | Term_bin (VEE, Term_ent (op_rl, id_rl), rr) -> (Term_bin (VEE, l, Term_ent (op_rl, id_rl)), rr)
  ...
  | Term_bin (VEE, ll, lr) -> (match r with
                                   Term_ent (op_r, id) -> (assoc_par_family_r l r)
...
let rec equiv_terms t ena_trans =
  (* gathering by "PhonyH-Cas/Par" and "PhonyT-Cas/Par" *)
  let rec gath_equ_trans equivs =
    match equivs with
      [] -> []
    | (e::es) -> (gath_equivs e false) @ (gath_equ_trans es)
  in
  let equivs = gath_equivs t false
  in
  if ena_trans then t::(gath_equ_trans (set_sub [t] equivs))
  else equivs
and gath_equivs t restrict =  
  ...
  let rec gath_equ_ph4 equivs =
    let phony = Term_una ( STAR, Term_ent (NIL, "") )
    in
    (match equivs with
       [] -> []
     | (e::es) -> Term_bin(WEDGE, phony, e) :: Term_bin(WEDGE, e, phony) :: Term_bin(VEE, phony, e) :: Term_bin(VEE, e, phony)
                  :: (gath_equ_ph4 es))
  in
  let equ_ph3 = (if (not restrict) then (gath_equ_ph3 equ_ph2) else equ_ph2) in
